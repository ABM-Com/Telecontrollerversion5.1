...
uint8_t waitResponseFlag;
uint16_t waitResponseTime = 0;
uint16_t waitResponseDuration = 150;
uint8_t stringCheckResult1 = 0;
uint8_t stringCheckResult2 = 0;
...
void loop() {
  ...
  if(ATbuffer != ""){
    SerialMon.println(ATbuffer.c_str());
    stringCheckResult =ATbuffer.find("MTI:");
    SerialMon.println(stringCheckResult);
    if(stringCheckResult != 255){
      ...
      SerialMon.println(smsIndex.c_str());
      delay(20);
      if(smsIndex != ""){
        smsIndex2 = static_cast<uint8_t>(atoi(smsIndex.c_str()));
        SerialMon.println(smsIndex2);
        if(smsIndex2 > 0) readSms();
        SerialMon.println("line 263");
      }
      SerialMon.println("sms have been read!");
    }
    ...
    if(stringCheckResult != 255){
      ...
      if(tempBuffer == "+DTMF: 1"){
        ...
        SerialMon.println("Not Detected");
        tempBuffer = "";
      }
      if(commandString != ""){
        ...
        SerialMon.print(tempBuffer.c_str());
        SerialMon.print("bm.cmd:");
        SerialMon.println(commandString.c_str());
  ...
  if(gsmCheckInterval >= 30 && busyFlag == 0) checkGsm();
  if(ioCheckInterval >= 3 && busyFlag == 0){ 
    Check_io();
  }
  //SerialMon.println("362 line");
  if(updateTimeInterval >= 50 && busyFlag == 0){
    SerialMon.println("line 364");
    checkSms();
    SerialMon.println("line 365");
    getDateTime();
    updateTimeInterval = 0;
  }
  if((waitResponseTime >= waitResponseDuration) && (waitResponseFlag > 0)){
    
    if(waitResponseFlag==2){
...
void initModule(){
  ...
  SerialMon.println("Checking inputs Status ...");
  Buzz(400,750);
  ...
  SerialMon.println("Checking Outputs Status ...");
...
string retriveGsmString(string atCommandString, uint8_t startAddress, uint8_t endAddress){
  ...
  if(ATbuffer != ""){
    trim(ATbuffer);
    SerialMon.println("641 line");
    SerialMon.println(ATbuffer.c_str());
    SerialMon.println(ATbuffer.length());
    int t1 = ATbuffer.find("+");
    if(t1 != 255){
      startAddress += t1;
    }
    string t = ATbuffer.substr(startAddress,endAddress);
    SerialMon.println(t.c_str());
    SerialMon.println("line 645");
    ATbuffer = "";
    SerialMon.println("line 648");
    return t;
  }
  return "";
}
...
void getDateTime(){
  if(!bootFlag){
    SerialMon.println("Initiliazing Date and Time ...");
  }
  if(gsmReady == 10){
    tempString = retriveGsmString("AT+CCLK?", 8, 18);
    SerialMon.println(tempString.c_str());
    SerialMon.println("line 662");
    if(tempString != ""){
      ...
      if(!bootFlag){
        ...
        SerialMon.print("    ");
        SerialMon.println(timeString.c_str()); 
      }
    Buzz(600, 950);
    }
  }
...
void getSignalLevel(){
  ...
  //SerialMon.print("signal level is :");
  //SerialMon.println(smsIndex.c_str());
  if(smsIndex != ""){
...
void sendSms(){
  ...
  if(cmdSource && (gsmReady == 10)){
    ...
    SerialAT.write((string("AT+CMGS=\"") + subscriberNumber +string("\"")).c_str());
    SerialAT.write(13);
    SerialAT.write(10);
    delay(200);
    SerialAT.write(Msg.c_str());
    SerialAT.write(13);
    SerialAT.write(10);
    SerialAT.write(26);
    ...
    if(restartTime > 2){
      ...
      SerialMon.println(subscriberNumber.c_str());
    }
    atCMD("AT");
    digitalWrite(ACT_PIN, !digitalRead(ACT_PIN));
    delay(1000);
...
void initGsm(){
  ...
  SerialMon.println("957 line");
  if(gsmReady == 10){
    SerialMon.println("960");
    getDateTime();    
    getGsmImei();
    Buzz(500,700);
    delay(100);
...
void readSms(){
  ...
  if(stringCheckResult != 255){
    ...
    SerialMon.println(localSenderNumber.c_str());
    SerialMon.println("this are data and time string :");
    SerialMon.println(dateString.c_str());
    SerialMon.println(timeString.c_str());

    if(protectFlag == 1){
      localTempString = localSenderNumber;
      SerialMon.println("line 1066");
      senderCheckResult = checkSender(localTempString);
    }else{
      senderCheckResult = 100;
    }
    SerialMon.println(senderCheckResult);
    if(senderCheckResult == 100){
      subscriberNumber = localSenderNumber;

      delay(10);
      tempBuffer = ATbuffer;
      SerialMon.println(1064);
      splitCommands();
      SerialMon.println(1082);
      SerialMon.println(Msg.c_str());
      if(echoFlag == 1 && Msg != ""){
        sendSms();
      }
      delay(1000);
...
void applyATCmd(){
  if(gsmReady == 10){
    ...
    delay(100);
    if(echoFlag == 1){
      SerialMon.println(atCmdString.c_str());
    }
    delay(1600);
    while(1){
        if(ATbuffer != ""){
...
void makeCall(){
  if(gsmReady == 10){
    ...
    Buzz(500,870);
    SerialMon.println("Calling Terminated!");
  }
}

void sendAlert(){
  SerialMon.println("line 1286");
  userIndex = usersSet.size();
  if(0 <= userIndex){
    tempValue = 0;
    ...
    stringLength = adminNumber.length();
    SerialMon.println(adminNumber.c_str());
    ...
    SerialMon.println("Not any user found!");
  }
}
...
int split(const string& source, string fillArray[], int startIdx, char delimiter, int maxSize) {
    ...
    while ((delimiterPos = source.find(delimiter, startPos)) != std::string::npos) {
        ...
        if (count + startIdx < maxSize) {
            ...
            SerialMon.println((source.substr(startPos, delimiterPos - startPos)).c_str());
            count++;
        } else {
  ...
  if(cmdString == "selfreset=on;"){
    ...
    if(userIndex > 0){
      ...
      for(tempValue =0;tempValue < userIndex;tempValue++){
        userNumber = usersSet[tempValue];
        SerialMon.println(userNumber.c_str());
        stringLength = userNumber.length();
    ...
    if((adminFlag == 100)||(protectFlag != 1)||(cmdSource == 0)){
      ...
      if(tempValue != 255){
        ...
        stringLength = userNumber.length();
        if(stringLength == 10){
          SerialMon.println("1475 line");
          userIndex = usersSet.size();
          SerialMon.println("1479 line");
          SerialMon.println(userIndex);
          delay(10);
  ...
  for(I=0;I<arrayCount;I++){
    ...
    if(stringCheckResult != 255){
      ...
      if((leftCharPos != 255)&&(rightCharPos != 255)){
        ...
        rightCharPos++;
        leftCharPos += 7;
        SerialMon.println("line 1579");
        commandString = tempBuffer.substr(leftCharPos,rightCharPos);
        SerialMon.println("line 1582");
        SerialMon.println(commandString.c_str());
        resultNumber = executeCommand(commandString);
      }
    }else{
      SerialMon.println("here in line 1709");
      stringCheckResult = tempBuffer.find("help");
      stringCheckResult1 = tempBuffer.find("inputs:");
      stringCheckResult2 = tempBuffer.find("outputs:");
      if(stringCheckResult != 255){
        getHelp();
      }else if(stringCheckResult1 != 255 && stringCheckResult2 != 255){
        SerialMon.println("in the place arrived");
        userNumber = usersSet[0];
        if((subscriberNumber == userNumber) && (waitResponseFlag > 0)){
          SerialMon.println("here in line 1719");
          uint8_t In1 = digitalRead(In1_PIN) == HIGH ? 1 : 0;
          uint8_t In2 = digitalRead(In2_PIN) == HIGH ? 1 : 0;
          uint8_t In3 = digitalRead(In3_PIN) == HIGH ? 1 : 0;
          
          SerialMon.println("ins are :");
          SerialMon.println(In1);
          SerialMon.println(In2);
          SerialMon.println(In3);

          leftCharPos = tempBuffer.find("outputs:");
          if(leftCharPos == 255) return;
          string outs = tempBuffer.substr(leftCharPos+12,3);

          SerialMon.println(outs.c_str());
          waitResponseTime = 0;

          uint8_t Rel1 = static_cast<uint8_t>(outs[2])-48;
          uint8_t Rel2 = static_cast<uint8_t>(outs[1])-48;
          uint8_t Rel3 = static_cast<uint8_t>(outs[0])-48;

          SerialMon.println("rels are :");
          SerialMon.println(Rel1);
          SerialMon.println(Rel2);
          SerialMon.println(Rel3);

          if(ioSetFlag == 30){
            if(In1 != Rel1 || In2 != Rel2 || In3 != Rel3  ){
              if(waitResponseFlag == 2){
                ...
                if (userNumber != "") {
                  ...
                  Msg = In1 ? "bm.cmd:rel1=on;" : "bm.cmd:rel1=off;";
                  Msg += In2 ? "bm.cmd:rel2=on;" : "bm.cmd:rel2=off;";
                  Msg += In3 ? "bm.cmd:rel3=on;" : "bm.cmd:rel3=off;";
                  Msg += "bm.cmd:read=io;";
            ...
            if(In1 == Rel1 || In2 == Rel2 || In3 == Rel3  ){
              if(waitResponseFlag == 2){
                ...
                if (userNumber != "") {
                  ...
                  Msg = In1 ? "bm.cmd:rel1=off;" : "bm.cmd:rel1=on;";
                  Msg += In2 ? "bm.cmd:rel2=off;" : "bm.cmd:rel2=on;";
                  Msg += In3 ? "bm.cmd:rel3=off;" : "bm.cmd:rel3=on;";
                  Msg += "bm.cmd:read=io;";
                  sendSms();
                  cmdSource = 0;
                }
    ...
    SerialMon.println("1597 line");
    SerialMon.println(Msg.c_str());
    SerialMon.println(executeResult.c_str());
    SerialMon.println(echoFlag);
    SerialMon.println(cmdSource);
...
void Check_io() {
  ...
  if (tempInStatus != inStatus) {
    ...
    if (userIndex != 0 && ioSetFlag >= 20) {
      ...
      SerialMon.println("line 1824");
      //sendAlert();
      SerialMon.println("line 1825");
    }
...
void rsp(string rspkind){

  subscriberNumber = persistence.getValue("adminnumber")->c_str();

  cmdSource = 1;
  Msg = string("Need to check the destination module!") + rspkind;
  sendSms();
  cmdSource = 0;
}