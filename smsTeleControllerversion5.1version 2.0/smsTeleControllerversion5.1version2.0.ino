...
void loop() {
  ...
  //check bufferAT for new response from GSM
  if(ATbuffer != ""){
    ...
    //if incoming call's phonenumber is authorize answer otherwise reject the call
    ...
    //check for incoming commands from dialpad of call
    ...
    //if atbuffer is not empty it means gsm module is working so we reset gsm module chek interval and earse atbuffer after process all data
    gsmCheckInterval = 0;
    ATbuffer = "";
  ...
  //aknowledg it is from serial monitor and process the commands
  //then erease the serial monitor buffer
  if(SMbuffer != ""){
...
//this function handles actions when user resets the relays
void checkRelays(){
...
//initilize module contains eeprom config loading
void initModule(){
...
//authorize phone number check if it is user or admin
...
//sends atcommand and slice the response and return it
...
//earase the blank spaces in the beginning or ending of string
...
void initGsm(){
  //turn on gsm module and check if it is responding or not and retrying
  ...
  //trying to connect to gsm network
  atCMD("ATE0");
  delay(50);
  digitalWrite(ACT_PIN, !digitalRead(ACT_PIN));
  loopCounter = 0;
...
//storing inputs and relays status in eeprom and variables
void writeIoStatus(){
...
//execute commands and return related response
...
//split commands from buffer one by one and excute them
void splitCommands(){
...
//check inputs and output status to detect changes and take proper actions
void Check_io() {
  tempInStatus = 0;
  uint8_t In1 = digitalRead(In1_PIN) == HIGH ? 1 : 0;
  uint8_t In2 = digitalRead(In2_PIN) == HIGH ? 1 : 0;
  ...
  /*SerialMon.print( );
  SerialMon.println(toBinary(tempInStatus).c_str());

  SerialMon.print("in:");
  SerialMon.println(toBinary(inStatus).c_str());

  SerialMon.print("outstatus:");
  SerialMon.println(toBinary(outStatus).c_str());*/
...
//this three function is for sending AT command to gsm module 
//they are taking diffrent inputs
...
//converting a value to binary form
